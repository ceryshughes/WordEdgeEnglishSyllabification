...
from convert_byu_long import read_byu_transcribed, consolidate_words
...
def get_counts():
    ...
    transcriptions = read_cmu_file_bh("CMUDictionaryEditedWithCELEXFrequenciesPublicVersion2.xls")
...
def predict_sylls_ind(cluster, onset_dist, coda_dist, normalize_conditional = True):
    predictions = {}
    ...
    for syll in syllabifications:
        if syll.coda != ['?']:
            ...
            if syll.cluster in prob_model:
                #print("Accessing cluster", syll.cluster, syll.coda, syll.onset)
                #print(syll.cluster, syll.coda, syll.onset)
                prob = prob_model[syll.cluster][(syll.coda, syll.onset)]
            else:
                print("Cluster not in model", cluster)
                prob = 0.25
            num = int(syll.numResponses)
            #print(syll.coda, "+", syll.onset, num, prob)
            if num > 0:
                #print(syll.cluster, prob, num, prob ** num, math.log(prob)*num)
                total += math.log(prob) * num
    return total
...
if __name__ == "__main__":
    ...
    onset_counts, coda_counts = get_counts()
    ...
    eddington_cluster_syllabs = read_byu_transcribed("BYU Syllabification Survey Transcriptions.csv", cmu_vowels, cmu_consonants, separate_words=separate_words)
    ...
    for cluster in eddington_cluster_syllabs.keys():
        if separate_words:
            word = cluster[0]
            cluster = cluster[1]
        pred = predict_sylls_ind(cluster, onset_dist_unsmoothed, coda_dist_unsmoothed)
        if separate_words:
    ...
    if loglikelihood:
        ...
        print("Edge",[predictions[prediction] for prediction in predictions])
        #print("Max",[onset_max_preds[cluster] for cluster in onset_max_preds])
        #print("Predictions",predictions)
        flat_data = []
        for cluster in eddington_cluster_syllabs:
            #print(cluster,  [syll for syll in eddington_cluster_syllabs[cluster]] )
            flat_data += [syll for syll in eddington_cluster_syllabs[cluster]]
        #flat_data = [syll for syll in eddington_cluster_syllabs[cluster] for cluster in eddington_cluster_syllabs]
        #print("Flat data", flat_data)
        print(data_log_likelihood(flat_data, predictions))
        print(data_log_likelihood(flat_data, onset_max_preds))
    else:
        if onehot:
