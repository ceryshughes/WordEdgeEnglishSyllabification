...
#Given the Eddington coprus filename (byu_fn),
...
def read_byu_transcribed(byu_fn, cmu_vowels, cmu_consonants, separate_words = False):
            ...
            if cluster:
                ...
                for code in [code for code in boundary_codes if len(code) <= len(cluster) + 1]:
                    ...
                    if not separate_words:
                        byu_dict[tuple(cluster)].append(MedialSyllabification(transcrip, cluster, coda, onset, boundary_code,
                                                              num_syllabifications, prop_syllabifcations))
                    else:
                        byu_dict[(orth, tuple(cluster))].append(MedialSyllabification(transcrip, cluster, coda, onset, boundary_code,
                                                              num_syllabifications, prop_syllabifcations))
